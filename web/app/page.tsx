...
import { Card, ErrorNote, Skeleton, Stat } from "@/components/ui";
import { api, ApiError } from "@/lib/api";
import { formatAge, formatVariance } from "@/lib/format";
import {
  MILESTONES,
  MILESTONE_LABEL,
  milestoneOf,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Delayed well portfolio</h1>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-ink-3">
            {running ? "Analysis running…" : `Last analysis ${formatAge(status?.age_hours)}`}
          </span>
          <button
            onClick={startRun}
            disabled={running || starting}
            className="h-8 rounded-md bg-accent px-3 text-xs font-medium text-white disabled:opacity-50"
          >
      ...
      {status?.schema_drifted && (
        <div
          role="status"
          className="rounded-xl border border-line bg-warn-soft px-5 py-3 text-sm text-warn"
        >
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Stat label="Slipped wells" value={wells === null ? "—" : stats.total} tone="danger" />
        <Stat
      ...
      <Card title="First failure by milestone" subtitle="Priority order — each well counted once">
          ...
          {MILESTONES.map((m) => {
            ...
            return (
              <div key={m} className="flex items-center gap-3">
                ...
                <span className="tnum w-10 shrink-0 text-right text-xs font-medium">{count}</span>
              </div>
            );
          })}
        </div>
      ...
      <Card
        title="Investigate a well"
        subtitle="Enter any well ID to run the task-delay query for it"
      >
